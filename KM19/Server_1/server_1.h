...
private:
    ...
    std::vector<std::vector<std::vector<element_array_type>>> Middle_Table0; // Level + Bucket + Position + element_array
    std::vector<std::vector<std::vector<element_array_type>>> Middle_Table1; // Level + Bucket + Position + element_array
    std::vector<element_array_type> Bottom_Table0;
    std::vector<element_array_type> Bottom_Table1;

    size_t get_tableLen(size_t level);
    void oblivious_hash(size_t level, size_t bucketID, std::vector<element_type>& obliv_table);
