...
namespace DPF {
    ...
    std::pair<std::vector<uint8_t>, std::vector<uint8_t>> Gen(size_t alpha, size_t logn) {
        /*
        Input: alpha = index, logn = log(array_length)
        Output: (ka, kb)
        ka (kb) = (16-byte s0) + (1-byte t0) + vector<CW (16-byte Scw, 1-byte tL, 1-byte tR)>
        */ 
        assert(logn <= 63);
        assert(alpha < (1UL<<logn));
...
//        std::cout << kb.hex() << std::endl;
        ...
        /*
        Store the final 7-bit information
        */
    ...
    bool Eval(const std::vector<uint8_t>& key, size_t x, size_t logn) {
        /*
        Input: key (i.e., ka or kb), x = index, logn = log(array_length)
        Output: ka or kb Eval on index x
        */
        assert(logn <= 63);
        assert(x < (1UL<<logn));
        block s;
    ...
    void EvalFullRecursive(const std::vector<uint8_t>& key, block s, uint8_t t, size_t lvl, size_t stop, std::vector<uint8_t>& res) {
        if(lvl == stop) {
            if(t) {
                reg_arr_union tmp;
                reg_arr_union CW;
                memcpy(CW.arr, key.data()+key.size()-16, 16); // final 16-byte
                tmp.reg = CW.reg ^ ConvertBlock(s);
